---
layout: default
title: D. pogonius
parent: Dorymyrmex
---

[Formicidae](../../../README.md) > [Dorymyrmex](../README.md) > pogonius

# *Dorymyrmex pogonius* — Ficha de especie

> **Hormiga de suelo endémica de Chile central** · Especie poco estudiada · Sin aguijón

---

## Clasificación

| Campo | Valor |
|-------|-------|
| Familia | Formicidae |
| Subfamilia | Dolichoderinae |
| Género | *Dorymyrmex* |
| Especie | *pogonius* Snelling, 1975 |
| Distribución natural | **Endémica de Chile** — Región Metropolitana a Región de Ñuble |
| Hábitat | Zonas abiertas y secas, pastizales; nidos en suelo |

---

## Morfología

Especie de polimorfismo débil o ausente — obreras de tamaño relativamente uniforme.

| Casta | Características |
|-------|-----------------|
| Obrera | **Negro** con cabeza de tono **ferrugíneo (rojizo)** — coloración bicolor distintiva |
| Macho | Cabeza notablemente rojiza; gran parte del tórax también rojizo |
| Reina | Documentada; coloración similar a obreras |

**Morfología detallada (Snelling, 1975):**
- Color negro con cabeza ferrugínea; a menudo gran parte del tórax también rojizo
- Setas superiores del psamóforo surgen desde un punto **por encima del margen inferior del foramen occipital**
- Frons y pronoto con al menos un par de setas largas y erectas
- Cara dorsal del primer tergo con setas erectas dispersas
- Ancho de la cabeza (HW) de al menos **0.8 mm**, generalmente más
- Par de setas frontales consistentemente **ausente**
- Setas más largas en el disco del pronoto ~0.5× el diámetro del ojo medio (MOD)
- Primer tergo con zona transversal preapical sin setas
- Margen inferior del fémur anterior sin setas erectas más allá del punto medio

---

## Historia del descubrimiento

Descrita por **Roy R. Snelling** en 1975, como parte de su revisión exhaustiva *The ants of Chile* (junto con J.H. Hunt). El nombre *pogonius* proviene del griego *pogon* (barba) — en referencia al psamóforo particularmente desarrollado de esta especie, cuyas setas superiores surgen por encima del margen inferior del foramen occipital.

---

## Comportamiento

| Rasgo | Descripción |
|-------|-------------|
| Agresividad | Baja — sin aguijón; se defienden con secreciones químicas |
| Actividad | Posiblemente **diurna** (género típicamente activo en zonas soleadas) |
| Forrajeo | **Cooperativo** — forman columnas de forrajeo (típico del género) |
| Formación de filas | Sí — las *Dorymyrmex* suelen formar columnas hacia fuentes de alimento |
| Defensa | Sin picadura; secreciones químicas defensivas |
| Escape | Bajo |
| Nido | Montículo irregular de tierra suelta en zonas abiertas sin vegetación |

---

## Biología

> ⚠️ **La biología de esta especie es poco conocida.** Los datos de cría son inferidos del comportamiento típico del género.

### Nidificación
- Nidos en **suelo**, en zonas abiertas sin cobertura vegetal
- Entradas coronadas por montículo irregular de tierra suelta — típico del género
- Hábitats secos y soleados

### Tamaño de colonia
- No documentado específicamente para *D. pogonius*
- Tamaño medio del género *Dorymyrmex*: **~5,000 individuos** (AntWiki)

---

## Alimentación

Basado en el comportamiento típico del género *Dorymyrmex* (omnívoro):

### Proteínas (2–3 veces por semana)
- Insectos pequeños vivos o muertos
- Carroña de artrópodos

### Azúcares (cada 2–3 días)
- Agua azucarada al 30% — opción principal, segura y siempre disponible
- Néctar artificial
- Melaza de pulgones (en naturaleza)
- Miel ecológica certificada diluida (opcional — solo si se tiene certeza de que está libre de pesticidas)

### Agua
- Siempre disponible. Bebedero con algodón húmedo.

---

## Parámetros de cría

Estimados según el hábitat natural (Chile mediterráneo central, zonas secas):

| Parámetro | Valor estimado |
|-----------|---------------|
| Temperatura nido | 20–26 °C |
| Humedad nido | 35–50% — especie de hábitat seco |
| Hibernación | Recomendable diapausa suave (12–16 °C, 2–3 meses) |

> Especie de zonas secas — evitar exceso de humedad en el nido.

---

## Nidificación en cautiverio

- Nidos de acrílico, yeso o impresos en 3D (PETG) con cámaras diferenciadas
- Tubo de ensayo para fundación
- Nido con gradiente seco — sin zona muy húmeda

**Ventilación:** Necesaria. Como Dolichoderinae, produce **secreciones químicas volátiles** (terpenoides) como defensa. Aunque no tiene aguijón, estos compuestos se acumulan en nidos cerrados. Asegurar flujo de aire pasivo.

---

## Dificultad de cría

| Criterio | Valoración |
|----------|-----------|
| Dificultad general | ⭐⭐ Intermedio |
| Velocidad de crecimiento | Desconocida |
| Resistencia | Desconocida |
| Espectacularidad | 🌟🌟🌟 (coloración negro/rojizo, montículos de nido llamativos) |

> ⚠️ **Especie endémica de distribución restringida.** No liberar individuos fuera de su rango natural (R. Metropolitana–Ñuble).

---

## Estado de conocimiento

Se necesitan estudios sobre:
- Dieta específica en naturaleza
- Tamaño real de colonias en Chile
- Ciclo reproductivo y vuelo nupcial
- Estrategia de fundación de la reina

---

## Referencias

- Snelling, R.R. (1975). Descripción de *Dorymyrmex pogonius*. En: Snelling, R.R. & Hunt, J.H. The ants of Chile (Hymenoptera: Formicidae). *Rev. Chil. Entomol.* 9: 63–129.
- AntWiki: [Dorymyrmex pogonius](https://www.antwiki.org/wiki/Dorymyrmex_pogonius)
- *Historia de las Hormigas de Chile* (fuente principal de este repositorio)
